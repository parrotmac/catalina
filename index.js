...
function nextRoundAvailable() {
  if(roundState.cardIndex + 1 < thingList.length) {
    return true
  }
  return false
}
...
function submitGuess(user, suspect, responseIndex, cardIndex) {
  if(roundState.cardIndex === cardIndex) {
    roundState.submissions[responseIndex].guesses.push({
      guesser: user,
      guessed: suspect
    });
  } else {
...
io.on('connection', function (socket) {
  ...
  socket.on('submit-guess', data => {
    ...
    submitGuess(user, suspect, responseIndex, cardIndex);
  });
